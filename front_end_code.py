...
filenames = ['Fdim7', 'Fm3_2', 'Fminor3', 'Gmin3']
iterations = 40
...
for filename in filenames:
    ...
    # read .wav file
    sample_rate, samples = wavfile.read('./input/'+filename+'.wav')
    ...
    song_times = np.arange(0,1.0*len(samples)/sample_rate*10+0.1,0.1) # time tracking 
    ...
    def neuron_tuning(s,fpref):
        lengths=len(s) # s is the x vector or different frequency values in a range going from -p 
    ...
    # Izhikevich model
    dv/dt = (0.04*(v**2) + 5*v + 140 - u + I + g*(Esyn-v) + gsynpois*(0.0-v))/ms : 1
    du/dt = a*(b*v - u)/ms : 1
    ...
    dgsynpois/dt=(-gsynpois/tau_syn)/ms :1
    '''
    neuron_reset = '''
    v = c
    u = u + d
    ...
    if(not os.path.isdir("./output")):
        os.mkdir("./output")
    if(not os.path.isdir("./output/"+filename)):
        os.mkdir("./output/"+filename)
    os.chdir("./output/"+filename)
    

    for fn in range(iterations):
        ...
        pNoise = PoissonInput(sensory_neurons, 'gsynpois', 30, 30*Hz, 0.005)
        ...
        net = Network(sensory_neurons, pNoise, SN, SPM)
        
        ## LOOP FOR ITERATING THROUGH song_times
        for t in range(len(song_times)):
        ...
        fout = open(filename+str(fn)+".dat", "wb")
        x = SPM.t/ms
        x = x.round().astype(int)
        y = array(SPM.i)
        pickle.dump(x, fout)
        pickle.dump(y, fout)
        pickle.dump([sensory_neurons.N, len(song_times)*10+1], fout)
        fout.close()
        ...
        del net, sensory_neurons, pNoise, SN, SPM
